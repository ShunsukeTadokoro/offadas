...
trait Tables {
  ...
  lazy val schema = Array(Login.schema, Offer.schema, OfferLog.schema, Offerset.schema, Status.schema, Test.schema, User.schema).reduceLeft(_ ++ _)
  ...
  case class OffersetRow(id: Int, userId: Int, name: Option[String])
  /** GetResult implicit for fetching OffersetRow objects using plain SQL queries */
  implicit def GetResultOffersetRow(implicit e0: GR[Int], e1: GR[Option[String]]): GR[OffersetRow] = GR{
    prs => import prs._
  ...
  class Offerset(_tableTag: Tag) extends Table[OffersetRow](_tableTag, "OFFERSET") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(userId), name).shaped.<>({r=>import r._; _1.map(_=> OffersetRow.tupled((_1.get, _2.get, _3)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val name: Rep[Option[String]] = column[Option[String]]("NAME", O.Length(45,varying=true))
  }
   ...
   *  @param id Database column ID SqlType(CHAR), PrimaryKey
   *  @param name Database column NAME SqlType(CHAR)
   *  @param password Database column PASSWORD SqlType(CHAR) */
  case class TestRow(id: String, name: String, password: String)
  /** GetResult implicit for fetching TestRow objects using plain SQL queries */
  implicit def GetResultTestRow(implicit e0: GR[String]): GR[TestRow] = GR{
    prs => import prs._
  ...
  class Test(_tableTag: Tag) extends Table[TestRow](_tableTag, "TEST") {
    def * = (id, name, password) <> (TestRow.tupled, TestRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(name), Rep.Some(password)).shaped.<>({r=>import r._; _1.map(_=> TestRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column ID SqlType(CHAR), PrimaryKey */
    val id: Rep[String] = column[String]("ID", O.PrimaryKey)
    /** Database column NAME SqlType(CHAR) */
    val name: Rep[String] = column[String]("NAME")
    /** Database column PASSWORD SqlType(CHAR) */
  ...
  case class UserRow(id: Int, email: String, password: Option[String], createdAt: java.sql.Timestamp, statusCode: String)
  ...
  class User(_tableTag: Tag) extends Table[UserRow](_tableTag, "USER") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(email), password, Rep.Some(createdAt), Rep.Some(statusCode)).shaped.<>({r=>import r._; _1.map(_=> UserRow.tupled((_1.get, _2.get, _3, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val password: Rep[Option[String]] = column[Option[String]]("PASSWORD", O.Length(255,varying=true))
